...
class TimeTable extends React.PureComponent<ChartProps, {}> {
    ...
    return (
      ...
      <div>
        ...
        {/* <div style={{ height: '30px'}} >
          <div style={{ width: parseInt(column.width, 10) || 300, zIndex: 1, position: 'relative', top: -25 }} >
            <div style={{ display: 'inline-block', width: '33.3%'}}>
              <div>|</div>
              <div >{entries[0].time}</div>
            </div>
            <div style={{ display: 'inline-block', width: '33.3%', textAlign: 'center'}}>
              <div >|</div>
              <div >{entries[ Math.ceil( (entries.length - 1) / 2)].time}</div>
            </div>
            <div style={{ display: 'inline-block', width: '33.3%', textAlign: 'right', paddingRight: 20}}>
              <div>|</div>
              <div>{entries[ entries.length - 1 ].time}</div>
            </div>
          </div>
        </div> */}
      </div>
  ...
  render() {
    const { className, height, columnConfigs, rowType } = this.props;
    const { data, rows } = this.props;
    // console.log( 'entries' , entries );
    // console.log( 'columnConfigs' , columnConfigs );
    // console.log( 'rows' , rows );   // 資料的 keys.
    ...
    for (let i = 0; i < rows.length; i++) {
      let tmp: any = {};
      tmp['metric'] = this.renderLeftCell(rows[i]);
      for (let j = 0; j < columnConfigs.length; j++) {
        if (columnConfigs[j].colType === 'spark') {
          tmp[columnConfigs[j].key] = this.renderSparklineCell(
            rows[i].label,
            columnConfigs[j],
            entries,
          );
        } else {
          tmp[columnConfigs[j].key] = this.renderValueCell(
            rows[i].label,
            columnConfigs[j],
            reversedEntries,
          );
    ...
    //for column base:
    // let arrayTemp: any = data
    // let array: any = Array.from(arrayTemp)
    // console.log('array', array);
    // console.log('arra cpuData',  array[0]['cpuData']  );

    // for( let i = 0 ; i < array.length ; i ++ ) {
    //   array[i]['cpuData'].sort(function (a: any, b: any) {
    //     return a.time > b.time ? -1 : -1;
    //    });
    //   array[i]['cpuData'] = this.renderSparklineCell( 'value', columnConfigs[0],  array[i]['cpuData'] );
    // }
    // console.log('array', array);
