...
function cellBarCht({
  ...
  const perc = Math.abs(Math.round((value / 100) * 100));
  let r, g, b;
  if (value < 60) {
    r = 34;
    g = 139;
    b = 34;
  } else if (90 > value && value > 60) {
    r = 255;
    g = 255;
    b = 0;
  } else {
  ...
  return (
    `linear-gradient(to right, rgba(${r},${g},${b},0.5), rgba(${r},${g},${b},1) ${perc}%, ` +
    `rgba(0,0,0,0.01) ${perc}%, rgba(0,0,0,0.001) 100%)`
  );
}
    ...
    (column: DataColumnMeta, i: number): ColumnWithLooseAccessor<D> => {
      ...
      return {
        ...
        Cell: ({ value }: { value: DataRecordValue }) => {
          if (!(column.key === 'CPU配置率' || column.key === '記憶體配置率')) {
