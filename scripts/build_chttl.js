...
function getPackages(packagePattern, tsOnly = false) {
  ...
  if (pattern === '*' && !tsOnly) {
    return `@chttl/!(${[...META_PACKAGES].join('|')})`;
  }
  if (!pattern.includes('*')) {
    pattern = `*${pattern}`;
  }
  const packages = [
    ...new Set(
      fastGlob
        .sync([
          `./node_modules/@chttl/${pattern}/src/**/*.${tsOnly ? '{ts,tsx}' : '{ts,tsx,js,jsx}'}`,
        ])
        .map(x => x.split('/')[3])
  ...
  return `@chttl/${packages.length > 1 ? `{${packages.join(',')}}` : packages[0]}`;
}
...
if (shouldCleanup) {
  ...
  const dirtyModules = 'node_modules/@types/react,node_modules/@chttl';
